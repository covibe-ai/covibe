...
- **业务 API**: `/api/v1/*`（由 covibe-server 处理）  
- **Happy API 代理**: `/happy-api/v1/*`（由反代/covibe-server 转发到 happy-server）  
...
```
POST /api/v1/wechat/pay/native    → 创建 Native 支付二维码
  Body: { order_id }
  → 调用微信支付 API → 返回 code_url（二维码内容）

POST /api/v1/wechat/pay/notify    → 微信支付回调（反向代理暴露到公网）
  → 验签 → 更新订单 → 触发履约
```
...
with connection.cursor() as cursor:
    cursor.execute("SELECT count(*) FROM \"Session\" WHERE \"accountId\" = %s AND active = true", [user.oidc_sub])
...
urlpatterns = [
    ...
    path('api/v1/', include('api_urls')),  # ViewSets
    path('happy-api/v1/', include('happy_proxy.urls')),  # 转发到 happy-server
]
```
...
# 测试 OIDC 登录
curl -X POST http://localhost:8000/api/v1/auth/oidc/login \
